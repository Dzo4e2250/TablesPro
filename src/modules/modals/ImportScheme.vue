...
-->
<template>
	<NcDialog v-if="showModal" size="normal" close-on-click-outside :name="t('tablespro', 'Import scheme')"
		...
		<div class="modal__content">
			...
			<div class="row">
				<div class="fix-col-4 end">
					<NcButton :aria-label="t('tablespro', 'Import')" type="primary" @click="actionSubmit">
						{{ t('tablespro', 'Import') }}
					</NcButton>
				</div>
			</div>
	...
	methods: {
		...
		uploadFile() {
			...
			reader.onload = (evt) => {
				...
				axios.post(generateOcsUrl('/apps/tablespro/api/2/tables/scheme'), json).then(async res => {
					if (res.status === 200) {
					...
					showError(t('tablespro', res.data?.message || 'Could not import data due to unknown errors.'))
				})
			}
