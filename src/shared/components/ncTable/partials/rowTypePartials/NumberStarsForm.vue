...
<template>
	<RowFormWrapper :title="column.title" :mandatory="isMandatory(column)" :description="column.description">
		<div class="align-center">
			<NcButton type="tertiary" :aria-label="t('tablespro', 'Reduce stars')" :disabled="column.viewColumnInformation?.readonly || localValue <= 0" @click="less">
				<template #icon>
					<Minus :size="20" />
				</template>
			</NcButton>
			...
			<NcButton type="tertiary" :aria-label="t('tablespro', 'Increase stars')" :disabled="column.viewColumnInformation?.readonly || localValue >= 5" @click="more">
				<template #icon>
					<Plus :size="20" />
				</template>
			</NcButton>
